...
class WalletController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'wallet_number.regex' => 'Wallet number may contain letters, numbers, and - _ / . characters.',
            'wallet_number.unique' => 'This wallet number is already registered.',
        ]);

        $validated['wallet_number'] = trim($validated['wallet_number']);
    ...
    public function update(Request $request, Wallet $wallet): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'wallet_number' => [
                ...
                'max:100',
                'regex:/^[A-Za-z0-9][A-Za-z0-9\-_\/.]*$/',
                Rule::unique('wallets', 'wallet_number')->ignore($wallet->id),
            ],
            'provider' => ['required', 'in:mtn,airtel,zamtel,other'],
            'currency' => ['required', 'string', 'size:3'],
            'opening_balance' => ['required', 'numeric', 'min:0'],
