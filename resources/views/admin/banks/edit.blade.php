...
@section('title', 'Edit Bank | '.config('app.system_name'))
...
@section('content')
    <div class="space-y-8">
        <div class="space-y-2 text-left">
            ...
            <p class="text-sm text-slate-400">{{ $bank->display_name }}</p>
        </div>

        <form action="{{ route('admin.banks.update', $bank) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
