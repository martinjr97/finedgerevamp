...
@section('title', 'Edit Market | '.config('app.system_name'))
...
@section('content')
    <div class="space-y-8">
        ...
        <form action="{{ route('admin.markets.update', $market) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $market->name) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $market->code) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="e.g., MKT-LUSAKA-001">
                        @error('code')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
            ...
            <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="address_line1" value="{{ old('address_line1', $market->address_line1) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                        @error('address_line1')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="address_line2" value="{{ old('address_line2', $market->address_line2) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                        @error('address_line2')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="city" value="{{ old('city', $market->city) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                        @error('city')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select name="province_id" id="province_id" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            ...
                            @foreach($provinces as $province)
                                <option value="{{ $province->id }}" @selected(old('province_id', $market->province_id) == $province->id)>{{ $province->name }}</option>
                            @endforeach
                        </select>
                        @error('province_id')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="district_id" id="district_id" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            ...
                            @foreach($districts as $district)
                                <option value="{{ $district->id }}" data-province-id="{{ $district->province_id }}" @selected(old('district_id', $market->district_id) == $district->id)>{{ $district->name }}</option>
                            @endforeach
                        </select>
                        @error('district_id')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
            ...
            <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="contact_person_name" value="{{ old('contact_person_name', $market->contact_person_name) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_person_phone" value="{{ old('contact_person_phone', $market->contact_person_phone) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="+260...">
                    ...
                    <div>
                        ...
                        <input type="email" name="contact_person_email" value="{{ old('contact_person_email', $market->contact_person_email) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                    ...
                    <div>
                        ...
                        <select name="portfolio_manager_id" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            ...
                            @foreach($portfolioManagers as $manager)
                                <option value="{{ $manager->id }}" @selected(old('portfolio_manager_id', $market->portfolio_manager_id) == $manager->id)>
                    ...
                    <div>
                        ...
                        <select name="loan_rate_type_id" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            ...
                            @foreach($loanRateTypes ?? [] as $rateType)
                                <option value="{{ $rateType->id }}" @selected(old('loan_rate_type_id', $market->loan_rate_type_id) == $rateType->id)>
                    ...
                    <div>
                        ...
                        <select name="branch_id" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" @selected(old('branch_id', $market->branch_id) == $branch->id)>
                                    {{ $branch->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="is_active" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                            <option value="1" @selected(old('is_active', $market->is_active) == true)>Active</option>
                            <option value="0" @selected(old('is_active', $market->is_active) == false)>Inactive</option>
                        </select>
                        @error('is_active')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
    ...
    <script>
        ...
        document.getElementById('province_id').addEventListener('change', function() {
            ...
            // Reset district selection if province changes
            if (provinceId !== "{{ old('province_id', $market->province_id) }}") {
        ...
        // Initialize district visibility on page load
