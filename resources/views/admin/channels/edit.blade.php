...
@section('title', 'Edit Channel | '.config('app.system_name'))
...
@section('content')
    <div class="space-y-8">
        ...
        <form action="{{ route('admin.channels.update', $channel) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $channel->name) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="e.g., Airtel Money">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $channel->code) }}" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="e.g., AIRTEL_MONEY">
                        @error('code')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="Optional description of this channel">{{ old('description', $channel->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_disburse" value="1" {{ old('can_disburse', $channel->can_disburse) ? 'checked' : '' }} class="w-5 h-5 rounded border-white/20 bg-white/5 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0">
                            <div>
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="can_repay" value="1" {{ old('can_repay', $channel->can_repay) ? 'checked' : '' }} class="w-5 h-5 rounded border-white/20 bg-white/5 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0">
                            <div>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="is_repayment_integrated" value="1" {{ old('is_repayment_integrated', $channel->is_repayment_integrated) ? 'checked' : '' }} class="w-5 h-5 rounded border-white/20 bg-white/5 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0">
                            <div>
            ...
            <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
                ...
                <div>
                    ...
                    <select name="is_active" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                        <option value="1" @selected(old('is_active', $channel->is_active) == true)>Active</option>
                        <option value="0" @selected(old('is_active', $channel->is_active) == false)>Inactive</option>
                    </select>
                    @error('is_active')
                        <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                    @enderror
