...
@section('title', 'Edit Physical Asset | '.config('app.system_name'))

@section('content')
    <div class="space-y-8">
        <div class="space-y-2 text-left">
            <p class="text-xs uppercase tracking-[0.4em] text-cyan-300">Financial Management</p>
            <h1 class="text-3xl font-bold">Edit Physical Asset</h1>
        </div>
        ...
        <form action="{{ route('admin.assets.update', $asset) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg space-y-4">
                <div>
                    ...
                    <input type="text" name="asset_type" value="{{ old('asset_type', $asset->asset_type) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="value" value="{{ old('value', $asset->value) }}" step="0.01" min="0" required
                               class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                        @error('value')
                            <p class="mt-1 text-sm text-rose-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="acquisition_date" value="{{ old('acquisition_date', optional($asset->acquisition_date)->format('Y-m-d')) }}"
                               class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">{{ old('description', $asset->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if ($asset->image_path)
                        <div class="mt-2 mb-3">
                            <img src="{{ asset('storage/'.$asset->image_path) }}" alt="{{ $asset->name }}" class="h-24 w-24 rounded-xl object-cover border border-white/10">
                        </div>
                    @endif
                    <input type="file" name="image" accept="image/*"
                           class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                    <p class="mt-1 text-xs text-slate-400">Leave empty to keep the current image (max 2 MB).</p>
                    @error('image')
                        <p class="mt-1 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="inline-flex items-center gap-2 text-sm text-slate-300">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-white/20 bg-white/10 text-cyan-400 focus:ring-cyan-500/30" @checked(old('is_active', $asset->is_active))>
                        Active
                    </label>
                </div>
            </div>
