...
@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            ...
            'description' => 'Register company physical assets and assign an optional custodian.',
            'buttons' => [[
                'action' => 'back',
                'text' => 'Back',
            ...
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="asset_type" value="{{ old('asset_type') }}" required placeholder="e.g. Furniture, Vehicle"
                        ...
                        @error('asset_type')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="value" value="{{ old('value') }}" step="0.01" min="0" required
                               class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                        @error('value')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="acquisition_date" value="{{ old('acquisition_date') }}"
                               class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                        @error('acquisition_date')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                ...
                @can('assets.create')
                    <p class="mt-3 text-xs text-slate-400">
                        Need a new custodian?
                        <a href="{{ route('admin.employees.create') }}" class="text-cyan-400 hover:text-cyan-300">Add employee</a>
                    </p>
                @endcan
            </div>

            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.assets.index') }}" class="inline-flex items-center gap-2 rounded-2xl border border-white/20 bg-white/5 px-4 py-3 text-base font-medium text-slate-300 hover:bg-white/10 transition">
                    Cancel
                </a>
