...
@section('title', 'Review Government Loan | ' . config('app.system_name'))

@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            'title' => 'Review Government Loan Application',
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg space-y-4">
                <h2 class="text-lg font-semibold text-white mb-2">Customer & Group</h2>
                <div class="space-y-3 text-sm text-slate-200">
                    ...
                    @if($customerGroup)
                        <div>
                            <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Customer Group</p>
                            <p class="text-base font-semibold text-white">
            ...
            {{-- Group Configuration --}}
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg space-y-4">
                <h2 class="text-lg font-semibold text-white mb-2">Government Group Configuration</h2>
                @if($customerGroup)
                    <div class="space-y-3 text-sm text-slate-200">
                        <div>
                            <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Maximum Loan Amount</p>
                            <p class="text-base font-semibold text-white">
                                {{ $customerGroup->max_loan_amount !== null ? number_format($customerGroup->max_loan_amount, 2) : 'Not set' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Maximum Tenure (Months)</p>
                            <p class="text-base font-semibold text-white">
                                {{ $customerGroup->max_loan_tenure_months ?? 'Not set' }}
                            </p>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Instalment Cross-over (%)</p>
                                <p class="text-base font-semibold text-white">
                                    {{ $customerGroup->instalment_cross_over_percentage !== null ? number_format($customerGroup->instalment_cross_over_percentage, 2) : 'Not set' }}
                                </p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Maximum Debit Ratio (%)</p>
                                <p class="text-base font-semibold text-white">
                                    {{ $customerGroup->maximum_debit_ratio !== null ? number_format($customerGroup->maximum_debit_ratio, 2) : 'Not set' }}
                                </p>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Loan Cut-off Day</p>
                                <p class="text-base font-semibold text-white">
                                    {{ $customerGroup->loan_cut_off_day ?? 'Not set' }}
                                </p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Loan Pay Date</p>
                                <p class="text-base font-semibold text-white">
                                    {{ $customerGroup->loan_payment_date ?? 'Not set' }}
                                </p>
                            </div>
                        </div>
                    </div>
                @else
                    <p class="text-sm text-slate-300">No customer group configuration available.</p>
                @endif
            </div>
        </div>
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-3 lg:grid-cols-4 text-sm text-slate-200">
                ...
                <div>
                    <p class="text-xs uppercase tracking-wide text-slate-400 mb-1">Amount Per Installment</p>
                    <p class="text-base font-semibold text-cyan-300">
                        ZMW {{ number_format((float) $installmentAmount, 2) }}
                    </p>
                </div>
                @endif
                <div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                ...
                <form method="POST" action="{{ route('admin.loan-applications.store-government', [$loanProduct, $customer]) }}">
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-500 to-indigo-600 px-4 py-3 text-base font-semibold text-white shadow-lg shadow-indigo-500/30 hover:from-indigo-600 hover:to-indigo-700 transition">
                        Confirm & Create Loan
                    </button>
                </form>
            </div>
