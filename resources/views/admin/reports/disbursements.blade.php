...
@section('title', 'Disbursements Report | '.config('app.system_name'))
...
@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            ...
            'buttons' => [
                [
                    'action' => 'export',
                    'text' => 'Export Details',
                    'href' => route('admin.reports.disbursements.export', request()->query()),
                ...
                [
                    'action' => 'export',
                    'text' => 'Export Summary',
                    'href' => route('admin.reports.disbursements.export-summary', request()->query()),
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4 shadow-lg">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <p class="text-xs font-medium text-slate-400 mb-1">Total Disbursements</p>
                        <p class="text-2xl font-bold text-white">{{ number_format($disbursements->total()) }}</p>
                    </div>
            ...
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4 shadow-lg">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <p class="text-xs font-medium text-slate-400 mb-1">Total Disbursed Amount</p>
                        <p class="text-xl font-bold text-emerald-400">
                            ZMW {{ number_format($disbursements->sum('principal_amount'), 2) }}
                        </p>
                    </div>
                    <div class="flex-shrink-0 ml-3">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <p class="text-2xl font-bold text-cyan-400">
                            {{ number_format($disbursements->filter(fn($loan) => $loan->disbursed_at && $loan->disbursed_at->isCurrentMonth())->count()) }}
                        </p>
                    </div>
                    <div class="flex-shrink-0 ml-3">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <p class="text-xl font-bold text-blue-400">
                            ZMW {{ number_format($disbursements->filter(fn($loan) => $loan->disbursed_at && $loan->disbursed_at->isCurrentMonth())->sum('principal_amount'), 2) }}
                        </p>
                    </div>
                    <div class="flex-shrink-0 ml-3">
                        <div class="w-12 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center">
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg">
            <form method="GET" action="{{ route('admin.reports.disbursements') }}" class="space-y-4">
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    {{-- Time From --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Time From</label>
                    ...
                    {{-- Time To --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Time To</label>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.reports.disbursements') }}" class="rounded-2xl border border-white/10 px-6 py-2 text-sm font-medium text-white/80 hover:bg-white/10 transition">
                        Clear Filters
                    </a>
                </div>
            </form>
                ...
                <table class="min-w-full w-full text-base text-slate-300">
                    <thead>
                        <tr class="text-base font-semibold uppercase tracking-[0.25em] text-white/80 text-center border-b-2 border-white/20">
                            ...
                            <th class="px-4 py-4 text-lg border-r border-white/10">Booked Loan Total</th>
                            <th class="px-4 py-4 text-lg border-r border-white/10">Disbursed At</th>
                    ...
                    <tbody>
                        @forelse ($disbursements as $loan)
                            <tr class="border-t border-white/40 text-center hover:bg-white/5 transition">
                                ...
                                <td class="px-4 py-4 border-r border-white/5">
                                    <span class="text-white">{{ $loan->channel->name ?? '—' }}</span>
                                    <div class="text-xs text-slate-400 mt-0.5">{{ $loan->disbursementChannelTypeLabel() }}</div>
                                </td>
                                <td class="px-4 py-4 border-r border-white/5 text-left text-sm text-slate-300">
                                    {{ $loan->disbursementDestinationSummary() ?: '—' }}
                                </td>
                                <td class="px-4 py-4 font-medium text-white border-r border-white/5">
                                ...
                                <td class="px-4 py-4 text-slate-400 border-r border-white/5">
                                    @if($loan->disbursed_at)
                                        ...
                                        <div class="text-sm text-slate-500">{{ $loan->disbursed_at->format('g:i A') }}</div>
                                    @else
                                        —
                                    @endif
                                </td>
