...
@section('title', 'Creditors | '.config('app.system_name'))
        ...
        @include('partials.admin.page-header', [
            ...
            'buttons' => [
                [
                    'action' => 'create',
                    'text' => 'Add Creditor',
                    'href' => route('admin.creditors.create'),
                    ...
                    'can' => auth('admin')->user()?->can('creditors.create')
                ]
            ]
        ])
                    ...
                    <tbody>
                        @forelse ($creditors as $creditor)
                            <tr class="border-t border-white/40 text-center hover:bg-white/5 transition">
                                <td class="px-4 py-4 font-medium text-white border-r border-white/5">{{ $creditor->name }}</td>
                                <td class="px-4 py-4 font-semibold border-r border-white/5">{{ number_format($creditor->amount, 2) }}</td>
                                <td class="px-4 py-4 border-r border-white/5">{{ $creditor->due_date ? $creditor->due_date->format('M d, Y') : '—' }}</td>
                                <td class="px-4 py-4 text-left border-r border-white/5">{{ Str::limit($creditor->description ?? '—', 50) }}</td>
                                <td class="px-4 py-4 border-r border-white/5">
                                    <span class="text-sm font-medium {{ $creditor->is_active ? 'text-emerald-400' : 'text-rose-400' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center text-slate-400">No creditors found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
