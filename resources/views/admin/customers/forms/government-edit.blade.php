...
<form action="{{ route('admin.customers.update', $customer) }}" method="POST" class="space-y-8">
    ...
    <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="ministry_id" required class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                    <option value="">Select Ministry</option>
                    @foreach ($ministries as $ministry)
                        <option value="{{ $ministry->id }}" @selected(old('ministry_id', $customer->ministry_id) == $ministry->id)>
                            {{ $ministry->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="deductions" value="{{ old('deductions', $customer->deductions) }}" step="0.01" min="0" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}" placeholder="0.00">
                @error('deductions')
                    <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2">
                ...
                <input type="text" name="work_address_line1" value="{{ old('work_address_line1', $customer->work_address_line1) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
            ...
            <div>
                ...
                <select
                    ...
                    class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}"
                >
                    <option value="">Select Province</option>
                    @foreach ($provinces as $province)
                        <option value="{{ $province->id }}" @selected(old('work_province_id', $customer->work_province_id) == $province->id)>
                            {{ $province->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                >
                    ...
                    @foreach ($districts as $district)
                        <option value="{{ $district->id }}" data-province-id="{{ $district->province_id }}" @selected(old('work_district_id', $customer->work_district_id) == $district->id)>
                            {{ $district->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="work_postal_code" value="{{ old('work_postal_code', $customer->work_postal_code) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
            ...
            <div>
                ...
                <input type="text" name="work_country" value="{{ old('work_country', $customer->work_country ?? 'Zambia') }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                @error('work_country')
                    <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
        <h2 class="mb-6 text-xl font-semibold {{ $headingClass }} flex items-center gap-2">
            <span class="w-1 h-6 rounded-full bg-{{ $colors['input_focus_border'] }}"></span>Address</h2>
        <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2">
            ...
            <div>
                ...
                <input type="text" name="city" value="{{ old('city', $customer->city) }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                @error('city')
                    <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="country" value="{{ old('country', $customer->country ?? 'Zambia') }}" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                @error('country')
                    <p class="mt-1 text-xs {{ $errorClass }}">{{ $message }}</p>
                @enderror
            </div>
    ...
    @include('partials.admin.customer-next-of-kin-fields', ['customer' => $customer])

    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('admin.customers.show', $customer) }}" class="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-4 py-3 text-sm {{ $headingClass }}">
            Cancel
        </a>
