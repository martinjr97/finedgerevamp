...
<form action="{{ route('admin.customers.update', $customer) }}" method="POST" class="space-y-8">
    ...
    <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="province_id" class="mt-2 w-full rounded-2xl {{ $inputClass }} text-white px-4 py-3 {{ $inputFocusClass }}">
                    ...
                    @foreach ($provinces as $province)
                        <option value="{{ $province->id }}" @selected(old('province_id', $customer->province_id) == $province->id)>
                            {{ $province->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="text-sm font-medium {{ $labelClass }}">State (Legacy Text)</label>
    ...
    <div class="rounded-3xl {{ $sectionClass }} p-6 shadow-lg">
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="next_of_kin_phone" value="{{ old('next_of_kin_phone') }}" maxlength="12" inputmode="numeric" pattern="260[0-9]{9}" class="w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40 zambian-phone-input" placeholder="260970000000">
