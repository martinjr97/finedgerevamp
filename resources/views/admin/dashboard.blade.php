...
@section('content')
    ...
    <div class="space-y-8" x-data="{ activeTab: 'today' }">
        ...
        @if ($newAssignedSupportTickets->isNotEmpty())
            <section>
                <article class="rounded-3xl border border-cyan-300/30 bg-cyan-500/10 p-6 shadow-lg space-y-4">
                    <div>
                        <h2 class="text-xl font-semibold text-cyan-100">New Support Ticket Assignments</h2>
                        <p class="text-sm text-cyan-200/80 mt-1">
                            These tickets were assigned to you and are waiting for your attention.
                        </p>
                    </div>

                    <div class="space-y-3">
                        @foreach ($newAssignedSupportTickets as $assignedTicket)
                            <div class="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-200">
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div class="space-y-2">
                                        <p class="font-semibold text-white">
                                            Ticket #{{ $assignedTicket->id }} · {{ $assignedTicket->subject }}
                                        </p>
                                        <p class="text-slate-300">
                                            Customer: {{ $assignedTicket->name }}
                                            @if($assignedTicket->assignedBy)
                                                · Assigned by {{ $assignedTicket->assignedBy->full_name }}
                                            @endif
                                        </p>
                                        <p class="text-xs text-slate-400">
                                            Assigned {{ $assignedTicket->last_assigned_at?->diffForHumans() ?? 'recently' }}
                                            · Status: {{ $assignedTicket->statusLabel() }}
                                        </p>
                                    </div>
                                    <a href="{{ route('admin.support-tickets.show', $assignedTicket) }}"
                                       class="inline-flex items-center gap-2 rounded-xl border border-cyan-300/40 bg-cyan-500/20 px-3 py-2 text-xs font-semibold text-cyan-100 hover:bg-cyan-500/30 transition">
                                        Open Ticket
                                    </a>
                                </div>
                            </div>
                    ...
                    <div>
                        <a href="{{ route('admin.support-tickets.index', ['assignment' => 'me']) }}"
                           class="inline-flex items-center gap-2 text-sm font-semibold text-cyan-200 hover:text-white transition">
                            View all tickets assigned to me
                        </a>
                    </div>
                </article>
            </section>
