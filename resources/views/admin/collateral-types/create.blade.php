...
@section('title', 'Create Collateral Type | '.config('app.system_name'))

@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            'title' => 'Create Collateral Type',
            'description' => 'Add a new collateral type for ' . $loanProduct->name,
            'buttons' => [
                [
                    'action' => 'secondary',
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg">
            <form method="POST" action="{{ route('admin.loan-products.collateral-types.store', $loanProduct) }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="4"
                              class="w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-2 focus:border-cyan-400 focus:ring-cyan-400/40">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
