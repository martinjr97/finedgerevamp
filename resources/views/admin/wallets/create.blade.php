...
@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            'title' => 'Add Wallet',
            'description' => 'Register a treasury mobile money or payment wallet account.',
            'buttons' => [[
                'action' => 'back',
                'text' => 'Back',
        ...
        <form action="{{ route('admin.wallets.store') }}" method="POST" class="space-y-6">
            ...
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="wallet_number" value="{{ old('wallet_number') }}" required maxlength="100" placeholder="e.g., 56654651" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40">
                        @error('wallet_number')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('provider')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('currency')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('opening_balance')<p class="mt-1 text-sm text-rose-400">{{ $message }}</p>@enderror
                    </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.wallets.index') }}" class="inline-flex items-center gap-2 rounded-2xl border border-white/20 bg-white/5 px-4 py-3 text-base font-medium text-slate-300 hover:bg-white/10 transition">
                    Cancel
                </a>
