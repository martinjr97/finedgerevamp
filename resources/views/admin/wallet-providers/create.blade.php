...
@section('title', 'Add Wallet Provider | '.config('app.system_name'))

@section('content')
    <div class="space-y-8">
        @include('partials.admin.page-header', [
            'title' => 'Add Wallet Provider',
            'buttons' => [
                ['action' => 'secondary', 'text' => 'Back', 'href' => route('admin.wallet-providers.index')],
            ],
        ])

        <form method="POST" action="{{ route('admin.wallet-providers.store') }}" class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg space-y-6 max-w-2xl">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code') }}" class="mt-2 w-full rounded-2xl bg-white/10 border border-white/10 text-white px-4 py-3 focus:border-cyan-400 focus:ring-cyan-400/40" placeholder="e.g. AIRTEL_MONEY">
