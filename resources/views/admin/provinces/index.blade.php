...
@section('title', 'Provinces | '.config('app.system_name'))
                ...
                <table data-datatable="true" class="min-w-full w-full text-base text-slate-300">
                    <thead>
                        <tr class="text-base font-semibold uppercase tracking-[0.25em] text-white/80 text-center border-b-2 border-white/20">
                            ...
                            <th class="px-4 py-4 text-lg border-r border-white/10">Country</th>
                    ...
                    <tbody>
                        @forelse ($provinces as $province)
                            <tr class="border-t border-white/40 text-center hover:bg-white/5 transition">
                                <td class="px-4 py-4 font-medium text-white border-r border-white/5">{{ $province->name }}</td>
                                <td class="px-4 py-4 border-r border-white/5">{{ $province->code }}</td>
                                <td class="px-4 py-4 border-r border-white/5">{{ $province->country }}</td>
                                <td class="px-4 py-4 border-r border-white/5">
                                    <span class="text-sm font-medium {{ $province->is_active ? 'text-emerald-400' : 'text-rose-400' }}">
                                        {{ $province->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="px-4 py-4">
                                    <div class="inline-flex items-center gap-3">
                                        <a href="{{ route('admin.provinces.edit', $province) }}" class="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-purple-500/40 to-indigo-500/40 border-2 border-purple-400/70 px-4 py-2 text-base font-semibold text-purple-200 hover:from-purple-500/60 hover:to-indigo-500/60 hover:border-purple-400 hover:text-white transition shadow-md shadow-purple-500/20">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-slate-400">No provinces found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
